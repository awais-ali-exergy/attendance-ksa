...
import { ExportSheet } from "react-xlsx-sheet-advance";
import { SC } from "../wasfaty/Api/serverCall";
import { Button } from "reactstrap";
import toast from "react-hot-toast";

import exportIconpng from "../../assets/wasfaty/export.svg";
...
class ExportExcel extends Component {
  ...
  getData = () => {
    ...
    SC.getCall({ url: endPoint, params: filter })
      .then((response) => {
        if (response.status === 200 && response.data.data) {
          if (this.props.onClick) {
            this.setState(
              ...
              () => this.props.onClick({ data: response.data.data })
            );
          } else {
            let { data, headers } = handleExportContent?.({
  ...
  render() {
    ...
    const { apiCall, fileName, headers, data, style = {} } = this.props;
          ...
          <div className="cursor-pointer">
            {loading ? (
              ...
              <img
                src={exportIconpng}
                onClick={
                  apiCall ? () => this.getData() : () => this.clickRef.click()
                }
              />
            )}
          </div>
